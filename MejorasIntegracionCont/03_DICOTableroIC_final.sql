...
---PROCEDURE----
/**************** xpDICOTableroIC ****************/
if exists (select * from sysobjects where id = object_id('dbo.xpDICOTableroIC') and type = 'P') drop procedure dbo.xpDICOTableroIC
GO
...
IF @Ejercicio IN ('(TODOS)',NULL,'','(Todos)')
    SELECT @Ejercicio=NULL

IF @Periodo IN ('(TODOS)',NULL,'','(Todos)')
    SELECT @Periodo=NULL
...
WHERE (YEAR(C.FechaContable)=CAST(@Ejercicio AS INT) OR ISNULL(@Ejercicio,'')='')
AND (MONTH(C.FechaContable)=CAST(@Periodo AS INT) OR ISNULL(@Periodo,'')='') 
