...
--EXEC xpDICOFixContReg '02',NULL,NULL,0


IF EXISTS(SELECT * FROM sysobjects WHERE TYPE='p' AND NAME='xpDICOFixContReg')
DROP PROCEDURE xpDICOFixContReg
GO
...
CREATE PROCEDURE xpDICOFixContReg
@Empresa	VARCHAR(5),
@Ejercicio	INT=NULL,
@Periodo	INT=NULL,
...
BEGIN
	...
	EXEC xpDICOTableroIC @Estacion,@Empresa,@Ejercicio,@Periodo,@Cuenta,1,0,@Silencio,0
	...
	IF EXISTS(SELECT 1 FROM DICOTableroIC WHERE Estacion=@Estacion)
		EXEC xpDICOTableroIC @Estacion,@Empresa,@Ejercicio,@Periodo,@Cuenta,0,1,@Silencio,0
