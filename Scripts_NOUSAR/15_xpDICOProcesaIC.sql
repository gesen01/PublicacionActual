...
GO
--EXEC xpDICOProcesaIC 999,'05',2023,11
IF EXISTS(SELECT * FROM sysobjects WHERE TYPE='p' AND NAME='xpDICOProcesaIC')
DROP PROCEDURE xpDICOProcesaIC
GO
CREATE PROCEDURE xpDICOProcesaIC
@Estacion	INT,
@Empresa	VARCHAR(10),
@FechaD		DATETIME,
@FechaA		DATETIME
...
AND c.FechaContable BETWEEN @FechaD AND @FechaA
--AND YEAR(c.FechaContable)=@Ejercicio
...
--SELECT * FROM #ContT_tbl ORDER BY ID
...
AND NOT EXISTS(SELECT 1 FROM ContReg c WHERE c.ID=ct.ID)

--SELECT * FROM #IntegracionCont_tbl WHERE ID=18692005 ORDER BY ID

INSERT INTO ListaSt(Estacion,Clave)
--Este caso valida que haya diferencia en el debe y haber entre contabilidad y contreg
SELECT @Estacion, i.ID
...
UNION ALL
SELECT @Estacion, ISNULL(i.ID,r.ID)
...
IF EXISTS(SELECT 1 FROM ListaSt AS ls WHERE ls.Estacion=@Estacion)
    EXEC xpDICOCorrecionIC @Estacion, @Empresa,0
