...
---PROCEDURE----
/**************** dbo.spDWHVerDesgloseCuentaExcel ****************/
if exists (select * from sysobjects where id = object_id('dbo.spDWHVerDesgloseCuentaExcel') and type = 'P') drop procedure dbo.spDWHVerDesgloseCuentaExcel
GO
...
* explorador*/
CREATE PROCEDURE dbo.spDWHVerDesgloseCuentaExcel
...
BEGIN
	...
	Select @Filtro='Contacto'
	Select @Cuenta = '210-102-000'
	--Select @Empresa = Empresa from empresa  where estatus = 'ALTA' and TieneMovimientos = 1 and CFD = 1
	Select @Sucursal = NULL
	Select @Moneda = 'Pesos'
	Select @FechaSaldoInicial =  convert (date,'1'+'/'+@PeriodoD+'/'+@Ejercicio)
